...
class ProviderRequestFormScreen extends StatelessWidget {
  ...
  const ProviderRequestFormScreen(
      {super.key,
  ...
  Map<String, dynamic>? _getFallbackSchemaForProviderRequest(
    ...
    return {
      ...
      'properties': {
        'voucherCode': {
          ...
          'description': 'Código associado ao voucher',
          'placeholder': 'Ex: 08013'
        },
        'serviceName': {
          'type': 'string',
          'label': 'Nome do Serviço',
          'description': 'Nome do serviço solicitado',
          'placeholder': 'Ex: Vistoria Veicular – Credenciada'
        },
        'status': {
          'type': 'string',
          'label': 'Status',
          'description': 'Status do pedido (scheduled, completed, etc.)',
          'placeholder': 'Ex: scheduled'
        },
        'result': {
          'type': 'string',
          'label': 'Resultado',
          'description': 'Resultado da inspeção (Aprovado, etc.)',
          'placeholder': 'Ex: Aprovado'
        },
        'inspector': {
          'type': 'string',
          'label': 'Inspetor',
          'description': 'Nome do inspetor responsável',
          'placeholder': 'Ex: Carlos Silva'
        },
        'inspectionDate': {
          'type': 'date',
          'label': 'Data da Inspeção',
          'description': 'Data da inspeção no formato ISO 8601 (YYYY-MM-DD)',
        ...
        'completedDate': {
          ...
          'description': 'Data de conclusão no formato ISO 8601 (YYYY-MM-DD)',
          'placeholder': 'dd/mm/aaaa'
        },
        'plate': {
          'type': 'string',
          'label': 'Placa',
          'description': 'Placa do veículo no padrão Mercosul (AAA1A23)',
        ...
        'renavam': {
          'type': 'string',
          'label': 'RENAVAM',
          'description': 'Registro Nacional do Veículo (9 a 11 dígitos)',
          'placeholder': 'Ex: 123456789'
        },
        'model': {
          'type': 'string',
          'label': 'Marca/modelo',
          'description': 'Marca e modelo do veículo',
        ...
        'observacoes': {
          'type': 'string',
          'label': 'Observações',
          'description': 'Observações sobre o pedido (até 1000 caracteres)',
  ...
  Widget _buildDynamicForm(Map<String, dynamic> formSchema, VouchersStore store,
      ...
      {GlobalKey<DynamicFormState>? formKey}) {
    return DynamicForm(
      key: formKey,
  ...
  Widget build(BuildContext context) {
    ...
    try {
      providerRequestObj = providerStore.providerRequests
          .firstWhere((r) => r.id == providerRequestId);
    } catch (_) {
          ...
          child: ConstrainedBox(
            ...
            child: Container(
              ...
              child: Builder(builder: (ctx) {
                ...
                final formFuture = ServicesService.instance.getServiceFormModel(
                    formId: providerRequestObj?.formId ?? '');
                  ...
                  children: [
                    ...
                    Row(
                      ...
                      children: [
                        ...
                        const Expanded(
                            child: DsText(
                                text: 'Formulário do Pedido do Provedor',
                                variant: DsTextVariant.titleVoucher)),
                        OutlinedButton(
                          onPressed: () {
                            final svcId = providerRequestObj?.serviceId ??
                                providerRequestObj?.voucherServiceId ??
                                '';
                            providerStore.setSelected(
                                providerRequestId: providerRequestId,
                                serviceId: svcId);
                            context.router.push(ProviderRequestQRCodeRoute(
                                providerRequestId: providerRequestId,
                                serviceId: svcId,
                          ...
                          Flexible(
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                DsText(
                                    text:
                                        'Código do Serviço: ${providerRequestObj?.serviceId ?? providerRequestObj?.voucherServiceId ?? ''}',
                                    variant: DsTextVariant.small),
                                const SizedBox(height: 8),
                                DsText(
                                    text:
                                        'Voucher nº: ${providerRequestObj?.voucherId ?? ''}',
                                    variant: DsTextVariant.small),
                                const SizedBox(height: 12),
                            ...
                            child: FutureBuilder<ValueResult<ServiceForm>>(
                              ...
                              builder: (context, snapshot) {
                                ...
                                if (snapshot.hasError) {
                                  final fallback =
                                      _getFallbackSchemaForProviderRequest(
                                          providerRequestId);
                                  return SingleChildScrollView(
                                      child: _buildDynamicForm(
                                          fallback!, store, context));
                                }
