...
import 'package:tsdtech_client_sdk/models/checkouts/checkout_request.model.dart' hide CardPaymentData;
...
import 'package:tsdtech_client_sdk/src/utils/card-utils/card_encryptor.dart' show CardPaymentData;
...
void main() {
  ...
  group('CheckoutOrchestrator.payWithCard()', () {
    test('fluxo completo retorna PaymentStatusResponse approved', () async {
      ...
      gatewayClient.dio.interceptors.add(MockGatewayInterceptor(
          (options, handler) {
    ...
    test('checkout falha retorna ValueResult.failure com contexto [checkout]',
        () async {
    ...
    test('fetchPublicKey falha retorna ValueResult.failure com [fetchPublicKey]',
        () async {
      checkoutAdapter.when('POST', '/checkouts/client', {
        'paymentMethod': 'card',
