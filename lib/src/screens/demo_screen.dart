...
class _DemoScreenState extends State<DemoScreen> {
  ...
  Widget _buildStandaloneFormsSection() {
    return SectionCard(
      ...
      subtitle: 'Use os blocos separadamente.',
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.stretch,
        children: [
          PaymentForm(
            ...
            onSubmit: (data) {
              ...
              _showMessage(
                data.isCard ? 'Cartão processado.' : 'PIX processado.',
              );
            },
          ),
          const SizedBox(height: 16),
          InfoTile(
            ...
            content: _paymentFormResult == null
                ? 'Nenhum submit ainda.'
                : _paymentFormResult!.method.name,
          ),
          const SizedBox(height: 20),
          CardForm(
            store: _cardFormStore,
            submitLabel: 'Validar cartão',
            onChanged: (data) => setState(() => _cardPreview = data),
            onSubmit: (data) {
              setState(() => _cardPreview = data);
              _showMessage('Cartão validado.');
            },
          ),
          const SizedBox(height: 16),
          InfoTile(
  ...
  Widget _buildThemeSection(TsdtechThemeData theme) {
    return SectionCard(
      title: 'Customização de tema',
      subtitle: 'Configurações em tempo real.',
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.stretch,
        children: [
          Container(
            ...
            child: Column(
              ...
              children: [
                ...
                PaletteRow(
                  label: 'Tema Atual',
                  value: _isDarkTheme ? 'Dark Mode' : 'Light Mode',
                ),
                PaletteRow(label: 'Brightness', value: theme.brightness.name),
              ],
            ),
          ),
          const SizedBox(height: 20),
          Wrap(
            ...
            runSpacing: 12,
            children: [
              ColorSwatchCard(label: 'Primary', color: theme.primaryColor),
              ColorSwatchCard(label: 'Accent', color: theme.accentColor),
              ColorSwatchCard(label: 'Surface', color: theme.surfaceColor),
            ],
          ),
        ],
      ),
