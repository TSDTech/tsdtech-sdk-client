...
class MyVouchersScreen extends StatelessWidget {
              ...
              if (vouchersStore.selectedTab == 0) {
                if (!vouchersStore.isLoading && vouchersStore.hasMore) {
                  vouchersStore.fetchMyVouchers(
                      page: vouchersStore.currentPage + 1, pageSizeParam: 10);
                }
              } else {
                if (!providerRequestStore.isLoading &&
                    providerRequestStore.hasMore) {
                  providerRequestStore.fetchProviderRequests(
                      page: providerRequestStore.currentPage + 1,
                      pageSizeParam: 10);
                }
              }
            }
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      child: Column(
                        ...
                        children: [
                          const Padding(
                            padding: EdgeInsets.fromLTRB(64, 31, 64, 31),
                            child: Column(
                              crossAxisAlignment: CrossAxisAlignment.start,
                              children: [
                                // Título com ícone
                          ...
                          // Tabs full width
                          VoucherTabs(
                              voucherStore: vouchersStore,
                              providerStore: providerRequestStore),
                          const SizedBox(height: 18),
                          Padding(
                            padding: const EdgeInsets.fromLTRB(64, 0, 64, 32),
                            child: Observer(
                                builder: (_) => _buildContent(context,
                                    vouchersStore, providerRequestStore)),
                          ),
                        ],
                      ),
                    ),
  ...
  Widget _buildVoucherList(BuildContext context, VouchersStore vouchersStore,
    ...
    final items = vouchers
        .map((voucher) => VoucherCard(
              voucher: voucher,
              onSchedule: () {
                context.router.push(MyVouchersFormRoute(
  ...
  Widget _buildContent(BuildContext context, VouchersStore vouchersStore,
      ProviderRequestStore providerRequestStore) {
    switch (vouchersStore.selectedTab) {
      case 0:
        ...
        return _buildVoucherList(
            context, vouchersStore, vouchersStore.filteredVouchers);
      case 1:
        if (providerRequestStore.isLoading &&
            providerRequestStore.providerRequests.isEmpty) {
          return const Center(
              child: SizedBox(
                  width: 48, height: 48, child: CircularProgressIndicator()));
        }
        return _buildProviderRequestList(context, providerRequestStore,
            providerRequestStore.filteredScheduledRequests);
      case 2:
        if (providerRequestStore.isLoading &&
            providerRequestStore.providerRequests.isEmpty) {
          return const Center(
        ...
        return _buildProviderRequestList(context, providerRequestStore,
            providerRequestStore.filteredCompletedRequests);
      default:
        if (vouchersStore.isLoading && vouchersStore.vouchers.isEmpty) {
          return const Center(
              child: SizedBox(
  ...
  Widget _buildProviderRequestList(
      ...
      return ProviderRequestCard(
        ...
        onViewDetails: () {
          ...
          if (st == 'FINISHED' || st == 'REJECTED' || st == 'COMPLETED') {
            context.router.push(
                ProviderRequestFinishedRoute(providerRequestId: request.id));
          } else {
            // Navigate to provider request form for pending/in-progress requests
            context.router.push(ProviderRequestFormRoute(
