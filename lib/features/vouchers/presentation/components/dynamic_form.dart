...
class DynamicFormState extends State<DynamicForm> {
  ...
  Widget build(BuildContext context) {
    ...
    final requiredFields =
        (widget.formSchema['required'] as List?)?.toSet() ?? {};
      ...
      if (type == 'select') {
        ...
        final options =
            (field['options'] as List?)?.map((e) => e.toString()).toList() ??
                (field['enum'] as List?)?.map((e) => e.toString()).toList() ??
                [];
          ...
          children: [
            ...
            DropdownButtonFormField<String>(
              ...
              items: options
                  ...
                  .toList(),
              onChanged: (v) =>
                  setState(() => widget.store.setFormValue(key, v ?? '')),
        ...
        children: [
          ...props.entries
              .map((e) => Padding(
                  ...
                  child: buildField(e.key, e.value as Map<String, dynamic>)))
              .toList(),
  ...
  Map<String, dynamic>? validateAndCollect() {
    ...
    for (final entry in props.entries) {
      ...
      final Map<String, dynamic> out = {
        ...
        'required':
            (widget.formSchema['required'] as List?)?.contains(id) ?? false,
      };
      if (field.containsKey('validationKey')) {
        out['validationKey'] = field['validationKey'];
      }
      if (field.containsKey('options')) {
        out['options'] = List.from(field['options'] as List);
      }
  ...
  List<Map<String, dynamic>> collectFormResponses(
      Map<String, dynamic> formSchema, Map<String, dynamic> values) {
  ...
  bool _isValidCPF(String input) {
    ...
    if (numbers.split('').every((c) => c == numbers[0])) {
      return false; // reject same-digit sequences
    }
